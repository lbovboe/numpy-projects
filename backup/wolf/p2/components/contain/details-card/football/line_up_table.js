      ...
      <div className="">
        {(homeLineUp || awayLineUp) && (
          <>
            <>
              <div
                className={`grid grid-cols-[2fr_1fr_2fr] items-center ${tableHeaderStyles} text-[12px] rounded-[10px]`}
              >
                <p className="text-start">
                  {homeLineUp?.data && breakUpFormation(homeLineUp?.formation)}
                </p>
                <p className="text-center text-[12px]">首發陣容</p>
                <p className="text-end">
                  {awayLineUp?.data && breakUpFormation(awayLineUp?.formation)}
                </p>
              </div>
              <div>
                {parsedLineUp.length > 0 &&
                  parsedLineUp.map(([home, away], i) => (
                    <div
                      key={i}
                      className="group grid grid-cols-2 items-center border-b border-solid border-border-color last:border-none whitespace-nowrap h-[47px] hover:text-pri-color"
                    >
                      <div className="flex items-center gap-x-[10px] justify-end pr-[8%]">
                        <p className="text-[#FF6600]">{home?.role_cn}</p>
                        <p className="">{home?.name}</p>
                        <Jersey number={home?.number} side="home" />
                      </div>
                      <div className="flex items-center gap-x-[10px] justify-end flex-row-reverse pl-[8%]">
                        <p className="text-[#6AA319]">{away?.role_cn}</p>
                        <p className="">{away?.name}</p>
                        <Jersey number={away?.number} side="away" />
                      </div>
                    </div>
                  ))}
              </div>
              {parsedBenchData.length > 0 && (
                <>
                  <div>
                    <div
                      className={`grid ${tableHeaderStyles} text-[12px] rounded-[10px]`}
                    >
                      替補陣容
                    </div>
                    {parsedBenchData.length > 0 &&
                      parsedBenchData.map(([home, away], i) => (
                        <div
                          key={i}
                          className="group grid grid-cols-2 items-center border-b border-solid border-border-color last:border-none text-center whitespace-nowrap h-[47px] hover:text-pri-color"
                        >
                          <div className="flex items-center gap-x-[10px] justify-end pr-[8%]">
                            <p className="text-[#FF6600]">{home?.role_cn}</p>
                            <p className="">{home?.name}</p>
