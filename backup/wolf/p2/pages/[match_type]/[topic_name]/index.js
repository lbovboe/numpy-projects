    ...
    if (tname.includes(".html")) {
      ...
      const otherType =
        matchType === "zuqiu"
          ...
          ? "zuqiu"
          : "all";
      const otherMatchBody = {limit: 5,
              state: "active"}
      if(otherType === "all"){
        otherMatchBody.important=1;
      }
      const [data1, data1Tdk, hotMatches, locData] = await Promise.all([
        fetch(url1, {
          method: "POST",
        ...
        fetch(
          process.env.API_DOMAIN + `/api/v1/match/${otherType}/?lang=zh_HANT`,
          {
            method: "POST",
            headers: process.env.API_HEADER,
            body: new URLSearchParams(otherMatchBody),
          }
        ),
      ...
      const processeddata1Tdk =
        data1Tdk.status == "200" && (await data1Tdk.json());
      const topicName =
        processedData1?.data?.competition?.topic_name || matchType;
      ...
      const APIsToCall = [
        fetch(
          process.env.API_DOMAIN + `/api/v1/articles/${topicName}?lang=zh_HANT`,
          {
            method: "POST",
            headers: process.env.API_HEADER,
            body: new URLSearchParams({
        ...
        fetch(
          process.env.API_DOMAIN + `/api/v1/match/${topicName}/?lang=zh_HANT`,
          {
            method: "POST",
            headers: process.env.API_HEADER,
            body: new URLSearchParams({
      ...
      const [matchVideos, endMatches, tdkData, data4] = await Promise.all(
        APIsToCall
      );
      const match_videos =
        matchVideos.status == "200" && (await matchVideos.json());
      ...
      const matchDetailsTdk =
        matchType === "zuqiu"
          ? processeddata1Tdk?.data?.list?.match_detail
          : processeddata1Tdk?.data?.list;
      const matchDetails =
        matchType === "zuqiu"
          ? processedData1?.data?.list?.match_detail
          : processedData1?.data?.list;
      const tdk = {};
      const competition =
        matchDetailsTdk.competition || processeddata1Tdk?.data?.competition;
      const competitionName =
        matchType == "wangqiu"
          ? competition.gb
          : processeddata1Tdk?.data?.list?.match_detail?.competition
              ?.name_abbr || matchDetailsTdk?.competition?.name_abbr;
      const HomeName =
        matchType == "wangqiu"
          ? matchDetailsTdk.is_single
            ...
            : matchDetailsTdk.PlayerA1.name_j +
              "/" +
              matchDetailsTdk.PlayerA2.name_j
      ...
      const AwayName =
        matchType == "wangqiu"
          ? matchDetailsTdk.is_single
            ...
            : matchDetailsTdk.PlayerB1.name_j +
              "/" +
              matchDetailsTdk.PlayerB2.name_j
      ...
      tdk.description =
        ...
        tdkrall.description
          .replace(
            /{{match.date}}/g,
            matchDetailsTdk.time ? tdkDate(matchDetailsTdk.time, "MM-DD") : ""
          )
      ...
      return {
        props: {
          ...
          otherType: otherType === "all" ? "hot" : otherType,
        },
      ...
      const queryParams1 = queryString.stringify({ topic: tname, count: 14 });
      ...
      const [matchdata, hotMatches, newsdata, tdkDatas] = await Promise.all([
        fetch(
          process.env.API_DOMAIN +
            `/api/v1/competition/${matchType}?${queryParams1}&lang=zh_HANT`,
          {
            method: "GET",
            headers: process.env.API_HEADER,
          }
      ...
      if (tname == "shijiebei") {
        const [rankdata, dataShooter, dataAssist] = await Promise.all([
          fetch(
            process.env.API_DOMAIN + `/api/v1/match/ranking/?lang=zh_HANT`,
            {
              method: "POST",
              headers: process.env.API_HEADER,
              body: new URLSearchParams({
          ...
          fetch(
            `${process.env.API_DOMAIN}/api/v1/worldcup/ranking/shooter?lang=zh_HANT&limit=15`,
            {
              method: "GET",
              headers: process.env.API_HEADER,
            }
          ...
          fetch(
            `${process.env.API_DOMAIN}/api/v1/worldcup/ranking/assists?lang=zh_HANT&limit=15`,
            {
              method: "GET",
              headers: process.env.API_HEADER,
            }
        ...
        if (matchType === "zuqiu") {
          shooter_data =
            dataShooter.status == 200 && (await dataShooter.json());
          assist_data = dataAssist.status == 200 && (await dataAssist.json());
        }
      }
      ...
      const processednewsdata =
        newsdata.status == 200 && (await newsdata.json());
