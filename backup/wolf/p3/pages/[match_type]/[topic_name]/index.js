...
export async function getStaticProps({ params }) {
  ...
  if (matchType == "404") {
    ...
    if (tname.includes(".html")) {
      ...
      const APIsToCall = [
        ...
        fetch(
          process.env.API_DOMAIN +
            `/api/v1/competition/match/recent/?lang=zh_HANT`,
          {
            method: "POST",
            headers: process.env.API_HEADER,
            body: new URLSearchParams({
      ...
      const [matchVideos, relatedMatches, tdkData, data4] = await Promise.all(
        APIsToCall
      );
      const match_videos =
        matchVideos.status == "200" && (await matchVideos.json());
      const related_matches =
        relatedMatches.status == 200 && (await relatedMatches.json());
      ...
      return {
        props: {
          ...
          relatedMatches: related_matches?.data?.list || null,
      ...
      const [matchdata, hotMatches, newsdata, videosData, tdkDatas] =
        await Promise.all([
          fetch(
            process.env.API_DOMAIN + `/api/v1/match/${tname}?lang=zh_HANT`,
            {
              method: "POST",
              headers: process.env.API_HEADER,
              body: new URLSearchParams({
      ...
      const hot_matches =
        hotMatches?.status == 200 && (await hotMatches?.json());
