      ...
      {data?.Statistic?.players && data?.Statistic?.players?.length > 0 && (
        <div className="player_statistics">
          {data?.Statistic?.players?.map((ele, i) => (
            <div
              key={i}
              className={`text-[14px] rounded-[10px] border border-solid border-border-color overflow-hidden ${i > 0 ? "mt-[15px]" : ""}`}
            >
              ...
              <div
                className={`grid grid-cols-[140px_repeat(12,1fr)] gap-x-[10px] text-[12px] ${tableHeaderStyles}`}
              >
                <p>球員</p>
                <p>位置</p>
                <p>時間</p>
