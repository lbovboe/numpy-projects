    ...
    <div className="">
      {data?.Statistic?.score && (
        <div className="rounded-[5px] overflow-hidden ">
          <div className={`grid grid-cols-[190px_repeat(9,1fr)] gap-x-[10px] ${tableHeaderStylesLanqiu}`}>
            <p>球隊</p>
            <p>
              1<sup>st</sup>
            </p>
          ...
          <div className="rounded-b-[10px]">
            {data?.Statistic?.score &&
              data?.Statistic?.score.map((val, idx) => (
                <div
                  key={idx}
                  className={`grid grid-cols-[190px_repeat(9,1fr)] gap-x-[10px] px-[10px] ${tableBodyStyles}`}
                >
                  <p>{val?.side == "home" ? data?.HomeTeam?.name : data?.AwayTeam?.name}</p>
