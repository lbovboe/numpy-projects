...
export default function TransferTable({ data, children }) {
  return (
    <Frame
      title="轉會情報"
    ...
    >
      {children}
      <div
        className={`grid grid-cols-[100px_1fr_1fr_20px_1fr_80px_80px] items-center justify-center gap-x-[20px] px-[15px] mt-[15px] rounded-[10px] text-white bg-pri-color h-[33px] text-[12px]`}
      >
        <div>日期</div>
        <div>球員</div>
        <div>来自</div>
          ...
          data.map((t, i) => (
            <div
              className={`group grid grid-cols-[100px_1fr_1fr_20px_1fr_80px_80px] gap-x-[20px]
                        ...
                        hover:text-sec-color whitespace-nowrap leading-[20px]`}
              key={i}
            >
              ...
              <div className="flex items-center justify-center">
                <svg
                  className="group-hover:fill-pri-color"
                  width="10"
                  height="12"
                >
