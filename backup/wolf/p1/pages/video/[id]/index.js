...
const allowedTopics = ["lanqiu", "zuqiu", "zonghe", "wangqiu"];
export async function getStaticProps({ params }) {
  ...
  if (id.includes(".html")) {
    ...
    const [data1, hotMatches, recomVideos, tdkVideoDetails, tdkDatas] =
      await Promise.all([
        fetch(url1, {
          method: "GET",
        ...
        fetch(
          process.env.API_DOMAIN +
            `/api/v1/articles/getarticle?${queryParams1}`,
          {
            method: "GET",
            headers: process.env.API_HEADER,
          }
    ...
    const tdk_video_details =
      tdkVideoDetails.status == 200 && (await tdkVideoDetails.json());
    const tdkRall = {};
    if (tdkDatas.status == 200) {
      const tdkData = await tdkDatas.json();
      tdkRall.title = tdkData?.data?.list?.Tdk.title.replace(
        /{{article.title}}/g,
        tdk_video_details?.data?.list?.video_details?.title
      );
    ...
    return {
      props: {
        ...
        pageType: "details",
        navTitle: processedData1?.data?.list?.video_details?.title,
        navTags: [["/video/", "集錦錄像"]],
      },
    ...
    return {
      props: {
        ...
        navTitle:
          tname === "all"
            ? "集錦錄像"
            : (processedData1?.data?.topic || getNewsTitle(tname)) + "集錦錄像",
        navTags: tname === "all" ? [] : [["/video/", "集錦錄像"]],
      },
      revalidate: Number(process.env.TIME_OUT_S),
    };
  }
