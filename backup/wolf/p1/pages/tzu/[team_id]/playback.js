...
export async function getStaticProps({ params }) {
  ...
  const tdkBody =
    isAlias.isTdk == "Y"
      ...
      : {
          url: "/tzu/" + teamId + "/playback/",
          pattern: "/tzu/[id]/playback/",
          id: teamId,
        };
  const [completeDataTdk, completeData, newsData,saishiData, tdkDatas] = await Promise.all([
  ...
  if (completeData.status != 200 || complete_matches.code == 50000) {
    LogWriter("/tzu/" + teamId + "/", completeData.status, complete_matches.code);
    return {
      revalidate: 60,
      redirect: {
        destination: "/",
  ...
  return {
    props: {
      ...
      navTags: [
        ["/tzu/", getNewsTitle(matchType)+"队"],
        ["/tzu/" + teamId + "/", complete_matches?.data?.topic],
      ],
      pageType: "team",
    },
